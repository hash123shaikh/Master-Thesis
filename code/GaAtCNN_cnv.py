...
path = 'F:/Dissertations/TOPIC SELECTION/Research paper/Read & Useful/pr_7_SiGaAtCNN/code/SiGaAtCNNstackedRF-master/Data/METABRIC/gatedAtnCnvOutput.csv'
...
dataset_cnv = numpy.loadtxt("F:/Dissertations/TOPIC SELECTION/Research paper/Read & Useful/pr_7_SiGaAtCNN/code/SiGaAtCNNstackedRF-master/Data/METABRIC/METABRIC_cnv_1980.txt", delimiter="\t")# Change the path to your local system
...
3. Splitting the CNV Dataset:

# This code splits the cnv dataset into training and testing sets for the current fold. 
  The training set (`x_train_cnv`, `y_train_cnv`) contains the data indexed by `train_index`, and the testing set (`x_test_cnv`, `y_test_cnv`) contains the data indexed by `test_index`.

4. Reshaping the CNV Data:

# The cnv data (`x_train_cnv` and `x_test_cnv`) is reshaped to add an additional dimension of size 1. 
...
5. `bias_init = keras.initializers.Constant(value=0.1)`: This line initializes a constant bias value of 0.1 using the `keras.initializers.Constant` initializer.
   The bias value will be used in the convolutional layers as part of the initialization process.

6. `main_input_cnv = Input(shape=(200,1),name='Input')`: This line defines the input layer for the CNN model. 
   The `Input` function creates a Keras input tensor with the shape `(200,1)`. 
   The first value (200) represents the number of time steps (features), and the second value (1) represents the number of channels. 
   In this case, it seems like the model expects input data with 200 features and one channel (perhaps for time-series data).
    """
    ...
    """
7. `conv_cnv1 = Conv1D(filters=num_of_filters, kernel_size=1, strides=2, padding='same', name='Conv1D_cnv1', kernel_initializer='glorot_uniform', bias_initializer=bias_init, activity_regularizer=l2(0.001))(main_input_cnv)`: This line creates the first convolutional layer (`conv_cnv1`). 
...
# `name='Conv1D_cnv1'`: This assigns a name to the layer for identification.
...
8. `gatedAtnConv_cnv1 = Conv1D(filters=num_of_filters, kernel_size=1, strides=1, padding='same', name='GatedConv1D1', activation='relu', kernel_initializer='glorot_uniform', bias_initializer=bias_init, activity_regularizer=l2(0.001))(conv_cnv1)`: This line creates a second convolutional layer (`gatedAtnConv_cnv1`) with a similar setup as the previous layer, but with a few differences:
...
9. `gatedAtnConv_cnv1_1 = Conv1D(filters=num_of_filters, kernel_size=3, strides=1, padding='same', name='GatedConv1D1_1', activation='relu', kernel_initializer='glorot_uniform', bias_initializer=bias_init, activity_regularizer=l2(0.001))(conv_cnv1)`: 
    This line creates a third convolutional layer (gatedAtnConv_cnv1_1) with a similar setup to gatedAtnConv_cnv1, but the kernel size is set to 3. 
    This layer also performs a 1x1 convolution, but it considers a slightly wider context of the input.    
    

10. `mult_1_1 = multiply([gatedAtnConv_cnv1, conv_cnv1])`: This line multiplies the output of `gatedAtnConv_cnv1` and `conv_cnv1` element-wise. 
     This is essentially introducing a gating mechanism where the output of `gatedAtnConv_cnv1` modulates the output of the previous convolutional layer (`conv_cnv1`).


11. `mult_1_1_1 = multiply([gatedAtnConv_cnv1_1, conv_cnv1])`: This line multiplies the output of `gatedAtnConv_cnv1_1` and `conv_cnv1` element-wise, introducing another gating mechanism with a slightly different context window.


12. `pooled_cnv1 = MaxPooling1D(pool_size=2, strides=1, padding='same')(mult_1_1)`: This line applies max-pooling to the output of mult_1_1. 
     Max-pooling with a pool size of 2 and stride 1 is used to downsample the data and reduce the dimensionality.

     
13. `pooled_cnv1_1 = MaxPooling1D(pool_size=2, strides=1, padding='same')(mult_1_1_1)`: This line applies max-pooling to the output of `mult_1_1_1`, creating another downsampled version of the data with a slightly different context window.
    
    
This part of the code defines the first part of a complex CNN model for processing cnv data. 
...
14. `merged = concatenate([pooled_cnv1, pooled_cnv2, pooled_cnv1_1, pooled_cnv2_2], name='merge', axis=1):
    This line creates a merged tensor by concatenating the outputs of four max-pooling layers: `pooled_cnv1`, `pooled_cnv2`, `pooled_cnv1_1`, and `pooled_cnv2_2`. 
...
15. `flat_cnv = Flatten(name='Flatten')(merged)`: This line applies a `Flatten` layer to the merged tensor. 
...
This helps the model combine information from different scales and contexts while maintaining a suitable format for further processing.
    ...
    plot_model(model, to_file='F:/Dissertations/TOPIC SELECTION/Research paper/Read & Useful/pr_7_SiGaAtCNN/code/SiGaAtCNNstackedRF-master/Data/METABRIC/cnv_gated_attention.png') # Change the path to your local system
    
    """
# This section of the code defines the fully connected layers of the CNV CNN model and assembles the complete model:

16. `dense_cnv = Dense(150, name='dense_cnv', activation='tanh', activity_regularizer=l2(0.001))(flat_cnv)`:
    This line creates a fully connected (dense) layer with 150 units/neurons.
    It takes the flattened output (`flat_cnv`) from the previous layer and applies the hyperbolic tangent (`tanh`) activation function.
    Additionally, L2 regularization with a coefficient of 0.001 is applied to the layer's output.

    
17. `drop_final1 = Dropout(rate=0.25)(dense_cnv)`: This line adds a dropout layer to the model. 
...
21. `model = Model(inputs=main_input_cnv, outputs=output)`: Here, the `Model` class is used to create the final model. 
    The inputs argument is set to `main_input_cnv`, which is the input layer defined earlier. 
...
In summary, this section defines the fully connected layers of the Cnv CNN model, including activation functions, dropout regularization, and output layer configuration. 
...
To summarize, `model.compile(loss='binary_crossentropy', optimizer='Adam', metrics=[sensitivity_at_specificity(Sp_value)])` sets up the model for training using binary cross-entropy as the loss function, Adam as the optimization algorithm, and sensitivity at a specific specificity level as the evaluation metric.         
    ...
    """
# A portion of the training data (x_train_cnv, y_train_cnv) is split further into a training set (x_train1, y_train1) and a validation set (x_val1, y_val1). 
...
# The trained model is evaluated on the testing data (x_test_cnv, y_test_cnv). 
...
25. `X_cnv = numpy.expand_dims(X_cnv, axis=2)`: This line expands the dimensions of the `X_cnv` dataset along the third axis. 
...
27. `intermediate_output = intermediate_layer_model.predict(X_cnv)`:
    This line uses the intermediate model to predict the output of the "dense_final3" layer for the entire `X_cnv` dataset. 
...
29. `y_pred = model.predict(X_cnv)`:
    This line uses the trained `model` to predict the outputs for the entire `X_cnv` dataset. 
    `y_pred` will contain the predicted probabilities for each input sample.

    
30. `fpr, tpr, thresholds = roc_curve(Y_cnv, y_pred, pos_label=1)`:
    This line computes the Receiver Operating Characteristic (ROC) curve by comparing the predicted probabilities (`y_pred`) to the true labels (`Y_cnv`). 
...
roc_auc = auc(fpr, tpr)
plt.plot(fpr,tpr, 'r', label = 'Gated_Attention_CNN-CNA = %0.3f' %roc_auc)
...
# `plt.plot(fpr, tpr, 'r', label='SiGaAtCNN-CNV = %0.3f' % roc_auc)`: This line plots the ROC curve using the False Positive Rate (FPR) on the x-axis and the True Positive Rate (TPR) on the y-axis. 
